package com.geekbrains.lesson9.repositories;
...
public class FileMetaProvider implements IFileMetaProvider {
    ...
    private static final String GET_META = "select hash from vma.file_info_metadata where hash = :hash";
    ...
    private static final String DELETE_FILE_META_DATA = "delete from vma.file_info_metadata where filename = :filename";
    ...
    @Override
    public int deleteMetaFile(UUID filename) {
        if (checkMetaFile(filename).isEmpty()) {
            return -1;
        }
        try (Connection connection = sql2o.open()) {
    ...
    private Collection<FileMetaDTO> checkMetaFile(UUID fileHash) {
        try (Connection connection = sql2o.open()) {
            return connection.createQuery(GET_META)
                    .addParameter("hash", fileHash)
